...
- **AI Chat:** `AiChatPanel` gửi `contextPrompt` (full `gameConfig` + hướng dẫn template vs behavior) tới `POST /api/projects/:id/generate`.

Tài liệu chi tiết: repo root **`docs/03-frontend/studio-editor/`** (đặc biệt `readme.md`, `behavior-runtime.md`) và **`SYSTEM_ARCHITECTURE.md`**.

## Scripts

```bash
npm install
npm run dev    # Vite dev server (thường :5173)
npm run build
npm run preview
```
...
Phần dưới là template Vite mặc định (ESLint nâng cao) — có thể bỏ qua khi làm việc với Studio.
